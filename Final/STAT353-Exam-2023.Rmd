...
# Build initial model
model1 <- glm(INCID18 ~ ., data = data_clean, family = negative.binomial(theta = 1.25))
summary(model1)
# Add nonlinearity term
model2 <- glm(INCID18 ~ C0534 + C0560  + FR_URBAN + I(C0534**2), data = data_clean, family = negative.binomial(theta = 1.25))
summary(model2)
model3 <- glm(INCID18 ~ C0534 + C0560  + FR_URBAN + I(C0534**2) + I(C0562*FR_LVEL), data = data_clean, family = negative.binomial(theta = 1.25))
summary(model3)
```  
...
(4). Nonlinearity term for percentage of students who feel safe at school (C0534^2)  
(5). Interaction term between student-to-teacher ratio (C0562) and school level (FR_LVEL)  
I employed a statistical technique known as negative binomial regression to construct the model. This method is suitable for count data, such as the total number of crimes in schools, and it addresses the problem of overdispersion, which occurs when the variance of count data is larger than the mean. I began by building an initial model that included all predictor variables. After that, I examined issues like multicollinearity and model assumptions and introduced nonlinearity and interaction terms to enhance the model's fit and interpretation.  
In summary, I consider the final model to be the best since it comprises predictor variables that are theoretically relevant and includes nonlinearity and interaction effects, which can improve the precision of the model's predictions. Additionally, the model is a good fit to the data, and the coefficients are interpretable in terms of their direction and size of influence on the outcome.  
